...
public class TestAssertions {
    ...
    public static void assertDeliveryAndInvoiceAddressesMatch(CheckoutPage checkoutPage) {
        ...
        Assert.assertEquals(deliveryAddress, invoiceAddress, "The delivery and invoice addresses don't match.");
    }

    public static void assertRegistrationAndCheckoutAddressesMatch(
            CheckoutPage checkoutPage, AccountRegistrationData registrationData) {
        Address expectedAddress = normalizeAddress(registrationData.getAddressRegistrationData());
        Address deliveryAddress = normalizeAddress(checkoutPage.getDeliveryAddress());
        Address invoiceAddress = normalizeAddress(checkoutPage.getInvoiceAddress());

        Assert.assertEquals(
                deliveryAddress, expectedAddress, "The delivery address doesn't match the registration address.");
        Assert.assertEquals(
                invoiceAddress, expectedAddress, "The invoice address doesn't match the registration address.");
    }
    ...
    public static void assertLoggedInUser(HomePage homePage, UserIdentityData userData) {
        ...
        Assert.assertEquals(currentUser, expectedUser, "Current logged in user and registration user don't match.");
    }
